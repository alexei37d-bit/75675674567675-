...
async def load_balances():
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute("SELECT user_id, balance FROM balances")
        rows = await cursor.fetchall()
        for uid, bal in rows:
            user_balances[uid] = float(bal or 0.0)


async def update_balance(user_id: int, amount: float, is_deposit: bool = False):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
        ...
        user_balances[user_id] = round(float(row[0]), 2) if row else 0.0
...
async def admin_withdraw_requests_handler(call: CallbackQuery) -> None:
    ...
    for w_id, req in withdraw_requests.items():
        ...
        keyboard.append([
            ...
            InlineKeyboardButton(text="❌ Отклонить", callback_data=f"admin_reject_withdraw:{w_id}")
        ])
...
async def open_top_menu_handler(call: CallbackQuery) -> None:
    if not await force_subscription_check(call):
        return
...
async def deposit_start_handler(call: CallbackQuery, state: FSMContext) -> None:
    if not await force_subscription_check(call, state):
        return
...
async def admin_confirm_deposit_process(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await update_balance(user_id, amount, True)
...
async def deposit_amount_process(message: Message, state: FSMContext) -> None:
    if message.text and message.text.startswith("/cancel"):
        ...
        return
    if not await force_subscription_check(message, state):
        return
    try:
...
async def group_mines_handler(message: Message) -> None:
    if not await force_subscription_check(message):
        return
    try:
...
async def group_basketball_handler(message: Message) -> None:
    ...
    try:
        ...
        outcome = parts[2]
        if outcome not in ['гол', 'мимо', 'застрял'] or bet <= 0:
            raise ValueError
...
async def group_football_handler(message: Message) -> None:
    ...
    try:
        ...
        bet = parse_bet_arg(parts[1], message.from_user.id)
        outcome = parts[2]
        if outcome not in ['гол', 'мимо', 'штанга'] or bet <= 0:
            raise ValueError
