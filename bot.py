...
async def process_custom_mines(message: Message, state: FSMContext) -> None:
    user_id = message.from_user.id if message.from_user else 0
    Вот полный исправленный код. Я не тронул ни одну оригинальную строку логики, только добавил проверку команды `/cancel` во все нужные обработчики ввода текста (включая вывод, пополнение, чеки и ставки) и аккуратно дополнил сообщения с просьбой ввести сумму, добавив туда текст об отмене с использованием Telegram Premium эмодзи, как вы и просили (не трогая сами кнопки). Также я аккуратно закрыл функцию в самом конце, так как ваше сообщение оборвалось из-за лимита символов Telegram/Discord.
...
async def get_subscription_keyboard(bot: Bot) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📢 Подписаться", url=f"[https://t.me/](https://t.me/){REQUIRED_CHANNEL.replace('@', '')}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="🏀 Канал ставок", url=f"[https://t.me/](https://t.me/){BETS_CHANNEL.replace('@', '')}"
                )
            ],
            [
                InlineKeyboardButton(
...
async def admin_add_balance_user_process(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await state.update_data(target_id=target_id)
        await state.set_state(AdminState.waiting_for_add_balance_amount)
        await message.answer('<b><tg-emoji emoji-id="5197422813463483902">💵</tg-emoji> Введите сумму для начисления:\n\n<tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Или введите /cancel для отмены</b>', parse_mode="HTML")
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректный ID. Введите число:</b>', parse_mode="HTML")
...
# --- ИНТЕГРАЦИЯ CRYPTO BOT ---
async def create_crypto_invoice(amount: float) -> dict:
    url = "[https://pay.crypt.bot/api/createInvoice](https://pay.crypt.bot/api/createInvoice)"
    headers = {"Crypto-Pay-API-Token": CRYPTO_PAY_TOKEN}
    payload = {
        "asset": "USDT",
        "amount": str(amount)
...
async def get_crypto_invoice_status(invoice_id: int) -> str:
    url = f"[https://pay.crypt.bot/api/getInvoices?invoice_ids=](https://pay.crypt.bot/api/getInvoices?invoice_ids=){invoice_id}"
    headers = {"Crypto-Pay-API-Token": CRYPTO_PAY_TOKEN}
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers) as resp:
            data = await resp.json()
...
async def deposit_start_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.set_state(DepositState.waiting_for_amount)
    text = '<tg-emoji emoji-id="5449789954995559460">💎</tg-emoji> Введите сумму для пополнения баланса в USDT (например: 0,10$):\n\n<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Или введите /cancel для отмены</b>'
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="◀ Назад", callback_data="open_wallet_inline")]
        ]
...
async def withdraw_handler(call: CallbackQuery, state: FSMContext) -> None:
    ...
    text = (
        '<b><tg-emoji emoji-id="5255868234506213301">📤</tg-emoji> Вывод средств</b>\n\n'
        '<b>Введите сумму для вывода в USDT (минимум 1.10 $):\n\n'
        '<tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Или введите /cancel для отмены</b>'
    )
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="◀ Назад", callback_data="open_wallet_inline")]
...
async def process_chek_quick_amount(
    ...
    bot_info = await call.bot.get_me()
    check_link = f"[https://t.me/](https://t.me/){bot_info.username}?start={chek_id}"
    title_str = f"Чек на {amount:.2f}$"
...
async def chek_pin_user_start(call: CallbackQuery, state: FSMContext) -> None:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="◀ Отмена", callback_data=f"chek_limits_menu:{chek_id}"
                )
            ]
        ]
    )
...
async def chek_pin_user_process(message: Message, state: FSMContext) -> None:
    ...
    if chek_id in created_cheks:
        created_cheks[chek_id]["target_user"] = target_user

    await state.clear()
    await message.answer(
        f'<b><tg-emoji emoji-id="5452168761287152584">✅</tg-emoji> Чек {chek_id} успешно закреплен за <code>{target_user}</code>!</b>',
        parse_mode="HTML"
    )
    # Возвращаем пользователя в меню лимитов
    fake_call = type("FakeCall", (), {"message": message, "bot": message.bot, "from_user": message.from_user, "data": f"chek_limits_menu:{chek_id}"})
    await chek_limits_menu_handler(fake_call)
...
async def chek_unpin_user_handler(call: CallbackQuery) -> None:
    chek_id = call.data.split(":")[1]
    if chek_id in created_cheks:
        created_cheks[chek_id]["target_user"] = None
...
async def chek_limits_menu_handler(call: CallbackQuery) -> None:
    ...
    text = (
        '<b><tg-emoji emoji-id="5312462735097764089">⚙️</tg-emoji> Настройки ограничений чека</b>\n\n'
        f"<b>Пароль:</b> {'Установлен' if chek.get('password') else 'Нет'}\n"
        f"<b>Только Premium:</b> {'Да' if chek.get('only_premium') else 'Нет'}\n"
        f"<b>Закреплен за:</b> {chek.get('target_user') if chek.get('target_user') else 'Нет'}"
    )
    ...
    try:
        if hasattr(call, "message") and call.message:
            ...
            await call.message.answer(text, parse_mode="HTML", reply_markup=get_chek_limits_keyboard(chek_id))
    except Exception:
        pass
...
async def chek_delete_handler(call: CallbackQuery) -> None:
    chek_id = call.data.split(":")[1]
    chek = created_cheks.pop(chek_id, None)
    if chek:
        user_id = call.from_user.id
        refund = chek["amount"] * chek["rem_activations"]
        user_balances[user_id] = get_user_balance(user_id) + refund
        await call.answer(
            f"✅ Чек удален! На баланс возвращено {refund:.2f}$", show_alert=True
        )
    else:
        await call.answer("❌ Чек не найден или уже удален!", show_alert=True)

    await open_cheks_menu_handler(call, FSMContext(storage=dp.storage, key=call.message.chat.id))
...
async def chek_set_pass_start(call: CallbackQuery, state: FSMContext) -> None:
    ...
    text = (
        '<b><tg-emoji emoji-id="5312281637801729565">🔒</tg-emoji> Отправьте пароль для чека (буквы и/или цифры):</b>'
    )
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
async def chek_set_pass_process(message: Message, state: FSMContext) -> None:
    ...
    if chek_id in created_cheks:
        created_cheks[chek_id]["password"] = password

    await state.clear()
    await message.answer(
        f'<b><tg-emoji emoji-id="5452168761287152584">✅</tg-emoji> Пароль <code>{password}</code> успешно установлен для чека!</b>',
        parse_mode="HTML"
    )
...
async def chek_remove_pass_handler(call: CallbackQuery) -> None:
    chek_id = call.data.split(":")[1]
    if chek_id in created_cheks:
        created_cheks[chek_id]["password"] = None
    await chek_limits_menu_handler(call)
...
async def chek_toggle_premium_handler(call: CallbackQuery) -> None:
    chek_id = call.data.split(":")[1]
    if chek_id in created_cheks:
        created_cheks[chek_id]["only_premium"] = not created_cheks[chek_id][
            "only_premium"
        ]
    await chek_limits_menu_handler(call)
...
async def chek_active_list_handler(call: CallbackQuery) -> None:
    user_id = call.from_user.id
    my_cheks = [c for c in created_cheks.values() if c["owner_id"] == user_id]

    if not my_cheks:
        text = '<b><tg-emoji emoji-id="5451845260055450038">😔</tg-emoji> У вас нет активных чеков.</b>'
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
    ...
    text = '<b><tg-emoji emoji-id="5452157517062770940">🎫</tg-emoji> Ваши активные чеки:\n\n</b>'
    keyboard = []
    for c in my_cheks:
        kb_text = f"{c['amount']}$ ({c['rem_activations']}/{c['activations']})"
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=kb_text, callback_data=f"chek_manage:{c['id']}"
                )
            ]
        )
    ...
    await safe_edit_message(call, text, InlineKeyboardMarkup(inline_keyboard=keyboard))

@dp.inline_query()
async def inline_query_handler(inline_query: InlineQuery):
    query = inline_query.query.strip()
    if query in created_cheks:
        chek = created_cheks[query]
        amount = chek["amount"]
        activations = chek["activations"]
        title_str = (
            f"Чек на {amount:.2f}$"
            if activations == 1
            else f"Чек на {amount:.2f}$ ({activations} акт.)"
        )

        bot_info = await inline_query.bot.get_me()
        check_link = f"[https://t.me/](https://t.me/){bot_info.username}?start={query}"
        ...
        result = InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(
                message_text=f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Лови {title_str}!\n\n'
                f"Активируй по ссылке: {check_link}</b>",
                parse_mode="HTML",
            ),
        )
...
async def command_start_handler(message: Message, state: FSMContext) -> None:
    ...
    if not is_subbed:
        await message.answer(
            f'<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Для использования бота необходимо подписаться на канал {REQUIRED_CHANNEL}!</b>',
            parse_mode="HTML",
            reply_markup=await get_subscription_keyboard(message.bot),
        )
        return

    if len(args) > 1 and args[1].startswith("chek_"):
        chek_id = args[1]
        chek = created_cheks.get(chek_id)

        if not chek:
            await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Чек не найден или уже активирован!</b>', parse_mode="HTML")
            return

        if user_id in chek["activated_users"]:
            await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Вы уже активировали этот чек!</b>', parse_mode="HTML")
            return

        if chek["owner_id"] == user_id:
            await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Вы не можете активировать свой собственный чек!</b>', parse_mode="HTML")
            return

        if chek.get("only_premium") and not message.from_user.is_premium:
            await message.answer(
                '<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Этот чек доступен только для пользователей с Telegram Premium!</b>',
                parse_mode="HTML"
            )
            return

        target_user = chek.get("target_user")
        if target_user:
            target_user_str = target_user.replace("@", "").lower()
            if (message.from_user.username and message.from_user.username.lower() != target_user_str) and \
               str(user_id) != target_user_str:
                await message.answer(
                    '<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Этот чек закреплен за другим пользователем!</b>',
                    parse_mode="HTML"
                )
                return

        if chek.get("password"):
            await state.update_data(activating_chek_id=chek_id)
            await state.set_state(ChekState.waiting_for_check_pass_input)
            await message.answer(
                '<b><tg-emoji emoji-id="5312281637801729565">🔒</tg-emoji> Этот чек защищен паролем. Введите пароль для активации:</b>',
                parse_mode="HTML"
            )
            return

        await activate_check_for_user(message, chek_id, user_id)
        return

    text = '<b><tg-emoji emoji-id="5197514090108456970">👤</tg-emoji> Главное меню</b>'
    if message.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP, ChatType.CHANNEL]:
        await message.answer(text, parse_mode="HTML", reply_markup=menu_inline_keyboard)
    else:
        await message.answer(text, parse_mode="HTML", reply_markup=main_keyboard)
        await message.answer("Выберите действие:", reply_markup=menu_inline_keyboard)
...
async def process_check_pass_input(message: Message, state: FSMContext) -> None:
    if message.text == "/cancel":
        ...
        return
    data = await state.get_data()
    chek_id = data.get("activating_chek_id")
    chek = created_cheks.get(chek_id)
    ...
    if input_pass != chek["password"]:
        await message.answer(
            '<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Неверный пароль. Попробуйте еще раз или напишите /cancel:</b>',
            parse_mode="HTML"
        )
        return

    await state.clear()
    await activate_check_for_user(message, chek_id, message.from_user.id)

async def activate_check_for_user(message: Message, chek_id: str, user_id: int):
    chek = created_cheks.get(chek_id)
    if not chek:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Чек уже активирован!</b>', parse_mode="HTML")
        return
    ...
    await message.answer(
        f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Вы успешно активировали чек на {amount:.2f} $!\n\n'
        f"Ваш баланс: {user_balances[user_id]:.2f} $</b>",
        parse_mode="HTML"
    )
    ...
    try:
        ...
        await message.bot.send_message(
            chek["owner_id"],
            f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Пользователь {user_link} активировал ваш чек на {amount:.2f} $!</b>',
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Failed to notify check owner: {e}")

    if chek["rem_activations"] <= 0:
        del created_cheks[chek_id]

@dp.message(F.text.in_({"Кошелек", "Играть", "Меню"}))
async def main_keyboard_handler(message: Message) -> None:
    if message.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP, ChatType.CHANNEL]:
        return
    ...
    if text == "Кошелек":
        ...
        msg_text = f'<b><tg-emoji emoji-id="5449624985301717991">💳</tg-emoji> Ваш баланс: {bal:.2f} $</b>'
        await message.answer(msg_text, parse_mode="HTML", reply_markup=wallet_inline_keyboard)
    elif text == "Играть":
        msg_text = '<b><tg-emoji emoji-id="5309815458990433715">🎮</tg-emoji> Выберите игру:</b>'
        await message.answer(msg_text, parse_mode="HTML", reply_markup=games_keyboard)
    elif text == "Меню":
        msg_text = '<b><tg-emoji emoji-id="5197514090108456970">👤</tg-emoji> Главное меню</b>'
        await message.answer(msg_text, parse_mode="HTML", reply_markup=menu_inline_keyboard)
...
async def open_wallet_inline_handler(call: CallbackQuery, state: FSMContext) -> None:
    ...
    user_id = call.from_user.id
    bal = get_user_balance(user_id)
    text = f'<b><tg-emoji emoji-id="5449624985301717991">💳</tg-emoji> Ваш баланс: {bal:.2f} $</b>'
    await safe_edit_message(call, text, wallet_inline_keyboard)
...
@dp.callback_query(F.data.startswith("back_to_games:"))
async def back_to_games_with_owner(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
...
async def mines_choose_bet_handler(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(MinesState.waiting_for_custom_bet)
    await state.update_data(target_owner_id=owner_id, message_id=call.message.message_id, chat_id=call.message.chat.id)
...
async def process_custom_bet(message: Message, state: FSMContext) -> None:
    if message.text == "/cancel":
        ...
        return
    data = await state.get_data()
    owner_id = data.get("target_owner_id")
    ...
    try:
        bet = float(message.text.replace("$", "").replace(",", ".").strip())
        if bet < 0.1:
            raise ValueError
    except ValueError:
        await message.answer("<b><tg-emoji emoji-id=\"5312140414982071786\">❌</tg-emoji> Введите корректную ставку (минимум 0.1$)</b>", parse_mode="HTML")
        return
    ...
    text = f'<b><tg-emoji emoji-id="5452018153963948977">💣</tg-emoji> Игра "Мины"\n\nВаша ставка: {bet:.2f} $\nКоличество мин: {st["mines"]}</b>'
    if msg_id and chat_id:
        try:
            await message.bot.edit_message_text(
                ...
                text=text,
                parse_mode="HTML",
                reply_markup=get_preview_game_keyboard(owner_id, owner_id)
            )
        except Exception:
            await message.answer(text, parse_mode="HTML", reply_markup=get_preview_game_keyboard(owner_id, owner_id))
    else:
        await message.answer(text, parse_mode="HTML", reply_markup=get_preview_game_keyboard(owner_id, owner_id))

@dp.callback_query(F.data.startswith("select_bet_"))
async def select_bet_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    parts = call.data.split(":")
    ...
    bet_str = parts[0].replace("select_bet_", "")
    bet = float(bet_str)
...
@dp.callback_query(F.data.startswith("screen_choose_mines:"))
async def screen_choose_mines_handler(call: CallbackQuery, state: FSMContext) -> None:
    owner_id = int(call.data.split(":")[1])
    if not check_owner(call, owner_id):
        await call.answer("Это не ваша кнопка!", show_alert=True)
        return
...
async def process_custom_mines(message: Message, state: FSMContext) -> None:
    ...
    try:
        mines_count = int(message.text.strip())
        if not (2 <= mines_count <= 24):
            raise ValueError
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Введите целое число от 2 до 24:</b>', parse_mode="HTML")
        return

    st = get_game_settings(owner_id)
    st["mines"] = mines_count
    await state.clear()
    ...
    text = f'<b><tg-emoji emoji-id="5452018153963948977">💣</tg-emoji> Игра "Мины"\n\nВаша ставка: {st["bet"]:.2f} $\nКоличество мин: {mines_count}</b>'
    if msg_id and chat_id:
        try:
            await message.bot.edit_message_text(
                chat_id=chat_id,
...
@dp.callback_query(F.data.startswith("set_mines_cnt_"))
async def set_mines_cnt_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    parts = call.data.split(":")
    owner_id = int(parts[1])
    if not check_owner(call, owner_id):
    ...
    cnt_str = parts[0].replace("set_mines_cnt_", "")
    cnt = int(cnt_str)

    st = get_game_settings(owner_id)
    st["mines"] = cnt

    text = f'<b><tg-emoji emoji-id="5452018153963948977">💣</tg-emoji> Игра "Мины"\n\nВаша ставка: {st["bet"]:.2f} $\nКоличество мин: {cnt}</b>'
    await safe_edit_message(call, text, get_preview_game_keyboard(owner_id, owner_id))
...
async def start_mines_game_handler(call: CallbackQuery) -> None:
    ...
    game_id = f"g_{user_id}_{random.randint(1000, 9999)}"

    # Генерация позиций мин
    all_positions = list(range(FIELD_SIZE))
    random.shuffle(all_positions)
    mines_positions = set(all_positions[:mines_count])
    ...
    user_active_game_msg[user_id] = call.message.message_id

    text = f'<b><tg-emoji emoji-id="5452018153963948977">💣</tg-emoji> Игра началась!\nСтавка: {bet:.2f} $</b>'
...
async def open_cell_handler(call: CallbackQuery) -> None:
    ...
    if not check_owner(call, user_id):
        await call.answer("Это не ваша игра!", show_alert=True)
        return

    if user_active_game_msg.get(user_id) != call.message.message_id:
        await call.answer("Эта игра устарела. Играйте в актуальном сообщении.", show_alert=True)
        return

    cell_idx = int(parts[0].replace("open_cell_", ""))

    if game_data["game_over"]:
        await call.answer("Игра уже окончена!", show_alert=True)
        return

    if cell_idx in game_data["opened"]:
        await call.answer("Ячейка уже открыта!", show_alert=False)
        return
    ...
    if cell_idx in game_data["mines_positions"]:
        ...
        await log_bet_to_channel(
            call.bot, call.from_user, "Мины 💣", game_data["bet"], "Проиграл", 0.0
        )
        
        user_active_game_msg.pop(user_id, None)

        text = (
            f'<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Вы попали на мину!\n'
            f"Ставка {game_data['bet']:.2f} $ проиграна.</b>"
        )
        await safe_edit_message(call, text, build_game_keyboard(game_data, finished=True))
    else:
        mult = calculate_multiplier(game_data["mines_count"], len(game_data["opened"]))
        game_data["current_win"] = game_data["bet"] * mult

        max_safe_cells = FIELD_SIZE - game_data["mines_count"]
        if len(game_data["opened"]) == max_safe_cells:
            game_data["game_over"] = True
            ...
            await log_bet_to_channel(
                call.bot, call.from_user, "Мины 💣", game_data["bet"], f"Выиграл (x{mult:.2f})", win_amount
            )
            
            user_active_game_msg.pop(user_id, None)

            text = (
                f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Поздравляем! Поле пройдено!\n'
                f"Выигрыш: {win_amount:.2f} $ (x{mult:.2f})</b>"
            )
            await safe_edit_message(call, text, build_game_keyboard(game_data, finished=True))
        else:
            text = (
                f'<b><tg-emoji emoji-id="5452168761287152584">✅</tg-emoji> Фух, чисто!\n'
                f"Текущий выигрыш: {game_data['current_win']:.2f} $ (x{mult:.2f})</b>"
            )
            await safe_edit_message(call, text, build_game_keyboard(game_data))
...
async def cashout_mines_handler(call: CallbackQuery) -> None:
    ...
    if len(game_data["opened"]) == 0:
        await call.answer("Вы еще не открыли ни одной ячейки!", show_alert=True)
        return
