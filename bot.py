...
async def start_tower_game_handler(call: CallbackQuery) -> None:
    ...
    if bet <= 0.15:
        if user_rigged_counters[user_id]["wins"] < 3:
            ...
            force_loss_floor = 0 # Lose immediately
    else:
        if random.random() < 0.90:
            force_loss_floor = random.randint(0, 2) # Lose early
