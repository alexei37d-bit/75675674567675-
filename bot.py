...
async def admin_add_balance_amount_process(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ БАГА С ДВОЙНЫМ НАЧИСЛЕНИЕМ
        # Сначала получаем актуальный баланс
        current_bal = get_user_balance(target_id)
        new_bal = current_bal + amount
        
        # Обновляем глобально и локально корректно
...
# --- НОВАЯ ФУНКЦИЯ: СПИСОК ЗАЯВОК НА ВЫВОД ---
...
# Клавиатура без нижних кнопок для группы
...
games_keyboard = InlineKeyboardMarkup(
    inline_keyboard=[
        ...
        [
            InlineKeyboardButton(
                text="Кубики",
                icon_custom_emoji_id="5451788978804001506",
                callback_data="dice_choose_bet",
            )
        ]
    ]
)
...
def get_bet_selection_keyboard(owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="✏️ Своя ставка", callback_data=f"custom_bet_mines:{owner_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_tower_bet_selection_keyboard(owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="✏️ Своя ставка", callback_data=f"custom_bet_tower:{owner_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_basketball_bet_selection_keyboard(owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="✏️ Своя ставка", callback_data=f"custom_bet_basketball:{owner_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_football_bet_selection_keyboard(owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text="✏️ Своя ставка", callback_data=f"custom_bet_football:{owner_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_dice_type_keyboard(bet: float, owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=f"Чет (x{DICE_COEFF})", callback_data=f"dice_bet_even_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"Нечет (x{DICE_COEFF})", callback_data=f"dice_bet_odd_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"Больше 3 (x{DICE_COEFF})", callback_data=f"dice_bet_more_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"Меньше 4 (x{DICE_COEFF})", callback_data=f"dice_bet_less_{bet}:{owner_id}")],
            [InlineKeyboardButton(text="🎲 2 Кубика", callback_data=f"dice_2_menu_{bet}:{owner_id}")],
...
def get_dice_2_menu_keyboard(bet: float, owner_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=f"2К Чет (x{DICE_COEFF})", callback_data=f"dice2_bet_even_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"2К Нечет (x{DICE_COEFF})", callback_data=f"dice2_bet_odd_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"2К Больше 7 (x{DICE_COEFF})", callback_data=f"dice2_bet_more_{bet}:{owner_id}")],
            [InlineKeyboardButton(text=f"2К Меньше 7 (x{DICE_COEFF})", callback_data=f"dice2_bet_less_{bet}:{owner_id}")],
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="XRocket", 
                    icon_custom_emoji_id="5451692831666112994",
                    callback_data="deposit_method_xrocket"
                )
            ],
            [
                InlineKeyboardButton(
                    text="CryptoBot", 
                    icon_custom_emoji_id="5452124248246096392",
                    callback_data="deposit_method_cryptobot"
                )
            ],
...
async def inline_check_handler(query: InlineQuery) -> None:
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"Активировать ({chek['rem_activations']}/{acts})",
...
async def complete_chek_activation(
    ...
    # ИСПРАВЛЕНИЕ Х2: Обновляем баланс только один раз
    ...
    if chek["amount"].is_integer():
        amount_str = str(int(chek["amount"]))
    text = (
        f'<b><tg-emoji emoji-id="5452157517062770940">💸</tg-emoji> Вы успешно активировали чек и получили {amount_str}$!</b>'
    )
    if isinstance(message_or_call, Message):
...
async def custom_bet_mines_handler(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        '<b><tg-emoji emoji-id="5197422813463483902">💵</tg-emoji> Введите свою ставку:</b>',
        parse_mode="HTML"
    )
...
async def process_custom_mines_bet(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await state.clear()
        await message.answer(
            '<b><tg-emoji emoji-id="5452018153963948977">💣</tg-emoji> Мины: Игровое поле</b>',
            parse_mode="HTML",
            reply_markup=get_preview_game_keyboard(message.from_user.id, message.from_user.id)
        )
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректная сумма.</b>', parse_mode="HTML")
...
async def process_custom_tower_bet(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await state.clear()
        await message.answer(
            '<b><tg-emoji emoji-id="5449397725697187601">🗼</tg-emoji> Башня: Игровое поле</b>',
            parse_mode="HTML",
            reply_markup=get_preview_tower_keyboard(message.from_user.id, message.from_user.id)
        )
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректная сумма.</b>', parse_mode="HTML")
...
async def process_custom_basketball_bet(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            ...
            parse_mode="HTML",
            reply_markup=get_basketball_type_keyboard(bet, message.from_user.id)
        )
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректная сумма.</b>', parse_mode="HTML")
...
async def process_custom_football_bet(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(
            ...
            parse_mode="HTML",
            reply_markup=get_football_type_keyboard(bet, message.from_user.id)
        )
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректная сумма.</b>', parse_mode="HTML")
...
async def process_custom_dice_bet(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await state.clear()
        await message.answer(
            f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики | Ставка: {bet:.2f} $\n\nВыберите режим:</b>',
            parse_mode="HTML",
            reply_markup=get_dice_type_keyboard(bet, message.from_user.id)
        )
    except ValueError:
        await message.answer('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Некорректная сумма.</b>', parse_mode="HTML")
...
async def open_cell_handler(call: CallbackQuery) -> None:
    ...
    # FIX: Rigged logic without adding visual mine bug
    if game_data.get("force_loss") and not is_mine:
        ...
        if unopened_mines:
            is_mine = True
            # Swap logic: make clicked cell a mine, move original mine elsewhere
...
async def start_tower_game_handler(call: CallbackQuery) -> None:
    ...
    active_tower_games[game_id] = {
        ...
        "current_win": 0.0,
        "game_id": game_id,
        "force_loss_floor": force_loss_floor,
...
async def dice_choose_bet_handler(call: CallbackQuery) -> None:
    ...
    if not check_owner(call, owner_id):
        await call.answer("Это чужая игра!", show_alert=True)
        return
    text = f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики: Выберите размер ставки:</b>'
...
async def select_dice_bet_handler(call: CallbackQuery) -> None:
    ...
    if not check_owner(call, owner_id):
        await call.answer("Это чужая игра!", show_alert=True)
        return
    text = f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики | Ставка: {bet_val:.2f} $\n\nВыберите режим:</b>'
...
async def dice_2_menu_handler(call: CallbackQuery) -> None:
    ...
    owner_id = int(parts[1])
    if not check_owner(call, owner_id): return
    text = f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> 2 Кубика | Ставка: {bet_val:.2f} $\n\nВыберите режим:</b>'
...
async def process_dice_game(call: CallbackQuery, bet_type: str, bet_val: float, owner_id: int, is_2_dice: bool = False):
    ...
    if is_2_dice:
        ...
        total = val1 + val2
        res = {'win': False, 'multiplier': DICE_COEFF}
        ...
        type_names = {'even':'Чет', 'odd':'Нечет', 'more':'Больше 7', 'less':'Меньше 7'}
    ...
    if res['win']:
        ...
        user_balances[user_id] = get_user_balance(user_id) + win_amount
        await update_balance(user_id, win_amount)
        await log_bet_to_channel(call.bot, call.from_user, "Кубики", bet_val, f"Победа ({t_name}) x{res['multiplier']} 🏆", win_amount)
        text = (
            f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики | Ставка: {bet_val:.2f} $ | {outcome_text}</b>\n\n'
            f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Поздравляем! Вы выиграли {win_amount:.2f} $ (x{res["multiplier"]})!</b>'
        )
    else:
        await log_bet_to_channel(call.bot, call.from_user, "Кубики", bet_val, f"Проигрыш ({t_name}) ❌")
        text = (
            f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики | Ставка: {bet_val:.2f} $ | {outcome_text}</b>\n\n'
            f'<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> К сожалению, вы проиграли {bet_val:.2f} $!</b>'
        )
...
async def start_handler(message: Message, state: FSMContext) -> None:
    ...
    if len(args) > 1:
        ...
        # Deep links for menu navigation
        if payload == "deposit":
            if not await force_subscription_check(message, state): return
            await deposit_start_handler(message, state) # Reuse handler logic roughly or redirect
            # Actually simpler to just simulate callback or call function
            # But since deposit_start expects CallbackQuery, let's just send the text/kb
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Пополнить",
                    icon_custom_emoji_id="5255805270285653933",
                    url=f"https://t.me/{(await message.bot.get_me()).username}?start=deposit"
                ),
                InlineKeyboardButton(
                    text="Вывести",
                    icon_custom_emoji_id="5255868234506213301",
                    url=f"https://t.me/{(await message.bot.get_me()).username}?start=withdraw"
                )
            ]
        ]
    )
...
async def group_mines_handler(message: Message) -> None:
    ...
    try:
        ...
        # Format: mines [mines_count] [bet] OR mines [bet]
        mines = 3
        bet = 0.1
        ...
        if len(parts) >= 3:
            ...
            # Could be just bet or just mines? Assume bet if float-like, else mines
            val = parts[1]
            try:
        ...
        # Start game immediately logic would require duplicating start_mines_game logic or mocking callback
        # For simplicity and stability, showing preview is safer, but user asked for "immediate start".
        # Let's mock a callback query object to reuse logic? No, too risky.
        # Just replicate the essential start logic here.
        user_balances[user_id] -= bet
        await update_balance(user_id, -bet)
        user_turnover[user_id] = get_user_turnover(user_id) + bet
        await update_global_stat('turnover_all', bet)
...
@dp.message(F.chat.type.in_([ChatType.GROUP, ChatType.SUPERGROUP]), F.text.lower().startswith(("башня", "tower", "т")))
...
async def group_dice_handler(message: Message) -> None:
    ...
    try:
        ...
        # куб чет 0.5
        # 2куб меньше 1
        keywords = ['чет', 'нечет', 'больше', 'меньше']
        ...
        # Direct play logic simulation via fake callback is hard, so we send menu or play directly?
        # User wants immediate play.
        # Let's construct a fake callback-like execution flow or just duplicate logic.
        # Duplicating logic for safety.
        ...
        if win:
            win_amt = bet * DICE_COEFF
            user_balances[user_id] += win_amt
            await update_balance(user_id, win_amt)
            await log_bet_to_channel(message.bot, message.from_user, "Кубики", bet, f"Победа ({btype}) 🏆", win_amt)
            res_text = f'<b><tg-emoji emoji-id="5449465422971711717">🎉</tg-emoji> Победа! {outcome_str}. Выигрыш: {win_amt:.2f}$</b>'
        else:
            await log_bet_to_channel(message.bot, message.from_user, "Кубики", bet, f"Проигрыш ({btype}) ❌")
            res_text = f'<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Проигрыш. {outcome_str}.</b>'
            
        await loading_msg.edit_text(f'<b><tg-emoji emoji-id="{DICE_EMOJI_ID}">🎲</tg-emoji> Кубики | {bet:.2f}$ | {btype}</b>\n\n{res_text}', parse_mode="HTML")
        
    except Exception:
        await message.reply('<b><tg-emoji emoji-id="5312140414982071786">❌</tg-emoji> Формат: куб [чет/нечет/больше/меньше] [ставка]</b>', parse_mode="HTML")
